...
class ZDT(Problem):
    ...
    def _evaluate(self, x, kx):#kx useless
        ...
        if self.options["type"] ==1 or self.options["type"] ==4:
            ...
            for i in range(ne):
                h[i,0] = 1 -np.sqrt(f1[i,0]/g[i,0]) - f1[i,0]/g[i,0]*np.sin(10*np.pi*f1[i,0])
        ...
        if kx == None :
            ...
            if self.options["type"] <5 :
                ...
                f1[:,0] = (kx ==0 )*(4*np.sin(6*np.pi*x[:,0])**6 - 36*np.pi*np.cos(6*np.pi*x[:,0])*np.sin(6*np.pi*x[:,0])**5) * np.exp(-4*x[:,0])
            ...
            if self.options["type"] < 4:
                ...
                for i in range(ne):
                    d_g[i,0] = (kx != 0)*(2*x[i,kx] + 40*np.pi*np.sin(4*np.pi*x[i,kx]))
            else :
                for i in range(ne):
                    d_g[i,0] = (kx != 0)*9/4/(nx-j)**.25/x[i,kx]**0.75
            
            d_h = np.zeros((ne,1), complex)
            if self.options["type"] ==1 or self.options["type"] ==4:
                for i in range(ne):
                    d_h[i,0] = - 0.5 *( g[i,0]*(kx == 0) - d_g[i,0]*(kx != 0))/g[i,0]**1.5 / x[i,0]**0.5
            elif self.options["type"] == 2 or self.options["type"] ==5:
                for i in range(ne):
                    d_h[i,0] = -2 *( g[i,0]*(kx == 0) - d_g[i,0]*(kx != 0))/g[i,0]**3 * x[i,0]                 
            else :
                for i in range(ne):
                    d_h[i,0] = - 0.5 *( g[i,0]*(kx == 0) - d_g[i,0]*(kx != 0))/g[i,0]**1.5 / x[i,0]**0.5
                    d_h[i,0] -= x[i,0]**2 *10*np.pi*np.cos(10*np.pi*x[1,0])/g[i,0]
                    d_h[i,0] -= (g[i,0]*(kx == 0) - d_g[i,0]*(kx != 0))*np.sin(10*np.pi*x[1,0])/g[i,0]**2
    ...
    def pareto(self,npoints = 300):
        ...
        if self.options["type"] == 3:
            F=[[0,0.0830015349],[0.4093136748,0.4538821041],   
