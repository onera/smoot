...
class MOO(SurrogateBasedApplication):
    ...
    def optimize(self, fun):
        ...
        if self.ny > 2 and self.options["criterion"] != "GA":
            self.log("Only GA is available for more than 2 objectives at the moment, criterion will be switched")
    ...
    def _find_best_point(self):
        ...
        if criter == "WB2S" :
            ...
            if EHVImax == 0:
                ...
                s = beta * sum([abs(moy(np.asarray(xmax).reshape(1, -1))[0][0]) for moy in moyennes]) / EHVImax
