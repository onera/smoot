...
class MOO(SurrogateBasedApplication):
    ...
    def _initialize(self):
        ...
        declare("criterion","PI",types=str,values=["PI","EHVI", "GA","WB2S"],
    ...
    def optimize(self, fun):
        ...
        try :
            ...
            print("EGO will be used as there is only 1 objective")
        ...
        if self.ny > 2 and self.options["criterion"] != "GA":
            print("Only GA is available for more than 2 objectives at the moment, criterion will be switched")
        ...
        self.result = minimize(self.probleme,NSGA2(pop_size=self.options["pop_size"]),("n_gen",self.options["n_gen"]),verbose=False)
        self.log("Optimization done, get the front with .result.F and the set with .result.X")
    ...
    def _setup_optimizer(self,fun):
        ...
        """
        if type(self.options["xdoe"]) == np.ndarray and type(self.options["ydoe"]) == np.ndarray:
    ...
    def _find_best_point(self):
        ...
        if criterion == "GA" :
            ...
            if var_x ==0 or var_f == 0 :
                return X[0,:]
            dispersion = [ q*(d_l_x[j] - µ_x)/var_x + (1-q)*(d_l_f[j] - µ_f)/var_f for j in range(X.shape[0])]
        ...
        if criterion == "WB2S" :
            ...
            if EHVImax == 0:
                ...
                s = beta * sum([abs(moy(xmax)[0][0]) for moy in moyennes]) / EHVImax
