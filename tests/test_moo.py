...
from smt.problems import Branin, Rosenbrock, ZDT
...
class TestMOO(SMTestCase):
    ...
    def test_rosenbrock_2Dto3D(self): 
        ...
        print("x_opt :",x_opt)
        print("y_opt :",y_opt)
        print("time taken Rosen : ", time.time() - start)
    ...
    def test_Branin(self):
        ...
        mo.optimize(fun=fun)
        x_opt, y_opt = mo.result.X[0] , mo.result.F[0]
        print("time taken Branin: ", time.time() - start)
        self.assertTrue(
    ...
    def ecart_front_inverse(y1,y2,fun,pts = 50):
        #pas exactement ce que je présente dans le diapo, car je prends un set
        #P et je fais la norme 2 et je prends le plus proche quoi qu'il arrive
        n=len(y1)
        #rotation 
        z1_train = [ (y1[i]-y2[i])/2**.5 for i in range(n)]
        z2_train = [ (y1[i]+y2[i])/2**.5 for i in range(n)]
        ...
        # comparison points
        z1 = [ random.uniform(-1, 1)/2**.5 for i in range(pts)]
        S = t.predict_values(np.asarray(z1))
        P = 10*pts #arbitrary, the bigger is the better
        _,y = fun.pareto(P)
        p1 = [ (y[0][i]-y[1][i])/2**.5 for i in range(P)]
        p2 = [ (y[0][i]+y[1][i])/2**.5 for i in range(P)]
    
        #dist
        delta = 0
        for i in range(pts):
            point = np.array([z1[i],S[i,0]])
            distances = [np.linalg.norm(point - np.array([p1[j],p2[j]])) for j in range(P) ]
            delta += min(distances)
        return delta/pts
    ...
    def test_zdt(self,type = 1, criterion = "PI", ndim = 2):
        ...
        self.assertAlmostEqual(0., dist, delta=0.5)
    ...
    def test_train_pts_known(self):
        ...
        yt = fun(xt)#idem : false "known" datapoint for training
        ...
        dist = TestMOO.ecart_front(y_opt1,y_opt2,fun)
        print("distance to the exact Pareto front" , dist)
        self.assertAlmostEqual(0., dist, delta=0.1)
    """    
